...
def main():
    ...
    if options == 'Home':
        ...
        if uploaded_file is not None:
            try:
                resume_bytes = PdfReader(uploaded_file)
                page = resume_bytes.pages[0] 
                resume_bytes = page.extract_text()
        
            except UnicodeDecodeError:
                resume_text = resume_bytes.decode('latin-1')

            with st.spinner("Processing your resume..."):
                #cleaned_resume = clean_resume(resume_bytes)
                input_features = tfidfd.transform([resume_bytes])
